...
function start_cluster() {
    if [ -z $2 ]; then
        ...
        if [ "$2" = "single" ]; then
            ...
            nohup ssh node0 "cd ~/tf ; python3 $1 --task_index=0" > serverlog-0.out 2>&1&
            nohup ssh node1 "cd ~/tf ; python3 $1 --task_index=1" > serverlog-1.out 2>&1&
            nohup ssh node2 "cd ~/tf ; python3 $1 --task_index=2" > serverlog-2.out 2>&1&
        fi
    fi
}